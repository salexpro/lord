(function () {
  ...
  var switchTab = function switchTab(activeTab) {
    document.querySelectorAll('.tabs-title').forEach(function (el) {
      el.firstChild.removeAttribute('aria-selected');
    });
    document.querySelectorAll('.tabs-panel').forEach(function (el) {
  ...
  function () {
    ...
    _createClass(Music, [{
      ...
      value: function () {
        ...
        /*#__PURE__*/
        regeneratorRuntime.mark(function _callee(artistId) {
          var response, data;
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                case 0:
                  _context.prev = 0;
                  _context.next = 3;
                  return fetch("".concat(this.url, "?entity=album&sort=recent&limit=6&id=").concat(artistId));
            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                case 0:
                  _context2.prev = 0;
                  _context2.next = 3;
                  return fetch("".concat(this.url, "?entity=song&limit=5&id=").concat(id));
      ...
      key: "renderAlbums",
      value: function renderAlbums(albums) {
        var albumsContainer = document.getElementById('albums');
        var albumsHtml = albums.reduce(function (html, album) {
          return "".concat(html, "\n            ").concat(album.wrapperType == 'collection' ? "<div class=\"music_album\" data-albumid=\"".concat(album.collectionId, "\">\n                    <img src=\"").concat(album.artworkUrl100.replace('100x100', '360x360'), "\" alt=\"").concat(album.collectionName, "\">\n                </div>") : '');
        }, '');
        albumsContainer.innerHTML = "<div class=\"music_albums\">".concat(albumsHtml, "</div>");
      }
    }, {
      key: "renderSongs",
      value: function renderSongs(songs) {
        var isFirstLoad = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        var songsContainer = document.getElementById('songs');
        var songsHtml = songs.reduce(function (html, song) {
          return "".concat(html, "\n            ").concat(song.wrapperType == 'track' ? "<div class=\"music_song\" data-preview=\"".concat(song.previewUrl, "\" data-songid=\"").concat(song.trackId, "\" data-cover=\"").concat(song.artworkUrl100.replace('100x100', '200x200'), "\" data-songname=\"").concat(song.trackName, "\">\n                    <div class=\"music_song_cover\">\n                        <img src=\"").concat(song.artworkUrl100.replace('100x100', '200x200'), "\" alt=\"").concat(song.trackName, "\">\n                        <button class=\"music_song_play\">\n                    </div>\n                    <div class=\"music_song_info\">\n                        <div class=\"music_song_title\">").concat(song.trackName, "</div>\n                        <div class=\"music_song_artist\">").concat(song.artistName, "</div>\n                    </div>\n                    <a class=\"button secondary\" href=\"").concat(song.trackViewUrl, "\" target=\"_blank\">iTunes</a>\n                </div>") : '');
        }, '');
  ...
  function () {
    ...
    _createClass(Player, [{
      ...
      value: function isWebAudio() {
        return typeof AudioContext !== 'undefined' || typeof webkitAudioContext !== 'undefined' || typeof mozAudioContext !== 'undefined'; // return false;
      }
    }, {
      key: "playPause",
      value: function playPause(forcePlay) {
        if (this.isWebAudio()) {
      ...
      value: function load(song) {
        if (this.isWebAudio()) {
          ...
          document.getElementById('playerPlayed').style.width = 0; // this.audio.play();
      ...
      key: "updateCurrTime",
      value: function updateCurrTime() {
        var currTime = this.isWebAudio() ? this.audio.getCurrentTime() : this.audio.currentTime;
        this.playerStartTime.innerText = "00:".concat(Math.floor(currTime) < 10 ? "0".concat(Math.floor(currTime)) : Math.floor(currTime));

        if (!this.isWebAudio()) {
          document.getElementById('playerPlayed').style.width = "".concat(Math.floor(100 * currTime / this.audio.duration), "%");
        } // fadeIn


        if (currTime <= 2) {
          var newVolume = (currTime / 2).toFixed(1);
        ...
        var fadePoint = this.isWebAudio() ? this.audio.getDuration() - 2 : this.audio.duration - 2;
        var duration = this.isWebAudio() ? this.audio.getDuration() : this.audio.duration;
        var currVolume = this.isWebAudio() ? this.audio.getVolume() : this.audio.volume; // Only fade if past the fade out point or not at zero already

        if (currTime >= fadePoint && currVolume > 0) {
          var _newVolume = ((duration - currTime) / 2).toFixed(1);

          if (this.isWebAudio()) {
            this.audio.setVolume(_newVolume);
          } else {
            this.audio.volume = _newVolume;
          }
      ...
      key: "setSongId",
      value: function setSongId(id) {
        this.songid = id;
      }
    }, {
      key: "getSongId",
      value: function getSongId() {
        return this.songid;
      }
    }, {
      key: "listen",
      value: function listen(event, func) {
        ...
        var pureEvent = this.isWebAudio() ? event : nativeEvent[event];
      ...
      key: "progressUpdate",
      value: function progressUpdate(progress) {
        var _this = this;

        if (this.isWebAudio()) {
          this.playerLoaded.style.width = "".concat(progress, "%");
          ...
          var progressChecker = setInterval(function () {
            ...
            var buffered = player.audio.buffered;

            if (buffered.length) {
              var loaded = 100 * buffered.end(0) / player.audio.duration;
              _this.playerLoaded.style.width = "".concat(loaded, "%");

              if (loaded == 100) {
                clearInterval(progressChecker);
                playerWaves.classList.add('is_loaded');
                _this.playerLoaded.style.width = 0;
              }
            }
          }, 200);
        }
      ...
      key: "seek",
      value: function seek(percent) {
        var time = percent * this.audio.duration / 100;
  ...
  var playerWaves = document.querySelector('.player_waves');
  var playButton = document.getElementById('playerButton');
  player.listen('loading', function (progress) {
  ...
  player.listen('finish', function () {
    playButton.classList.remove('is_playing');
    document.querySelectorAll('.music_song').forEach(function (el) {
      return el.classList.remove('is_playing');
    });
  });
  player.listen('audioprocess', function () {
    player.updateCurrTime();
    var currSong = document.querySelector("[data-songid=\"".concat(player.getSongId(), "\"]"));
    if (currSong) currSong.classList.add('is_playing');
  });

  if (player.isWebAudio()) {
    player.audio.on('seek', function () {
      player.audio.setVolume(1);
    });
  } else {
    document.getElementById('waveform').addEventListener('click', function (e) {
      var percent = (e.clientX - this.parentNode.offsetLeft) / this.offsetWidth * 100;
      player.seek(percent);
    });
  }
  ...
  if (songs) {
    songs.addEventListener('click', function (e) {
      if (e.target.className === 'music_song_play') {
        var currSong = e.target.closest('.music_song');
        ...
        document.querySelector("[data-songid=\"".concat(player.getSongId(), "\"]")).classList.add('is_playing');
      }
    });
  }
  ...
  regeneratorRuntime.mark(function _callee2() {
    var music, artistId, albums, songs;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            music = new Music('https://itunes.apple.com/lookup');
            artistId = 329938300;
            _context2.prev = 2;
            _context2.next = 5;
            return music.getAlbums(artistId);

          case 5:
            albums = _context2.sent;
            music.renderAlbums(albums.results);
            _context2.next = 9;
            return music.getSongs(artistId);

          case 9:
            songs = _context2.sent;
            music.renderSongs(songs.results, true);
            document.querySelector('.music_albums').addEventListener('click',
            /*#__PURE__*/
            function () {
                  ...
                  while (1) {
                    switch (_context.prev = _context.next) {
                      case 0:
                        ...
                        albumId = e.target.getAttribute('data-albumid');
                        ...
                        _context.next = 6;
                        return music.getSongs(albumId);

                      case 6:
                        _songs = _context.sent;
                        e.target.classList.remove('is_loading');
                        music.renderSongs(_songs.results);
